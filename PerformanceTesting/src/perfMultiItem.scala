...
object perfMultiItem {
  ...
  def main(args: Array[String]) {
    ...
    val datasets = Array("test", "BIBLE", "Kosarak", "slen1", "slen2").drop(startDataSet)
    ...
    val pw = new PrintWriter(new File(getCurrentDirectory + "\\results\\sparkPerfMesurementMULTI.txt" ))
    ...
    datasets.foreach(fileName => {
      ...
      listSupport.foreach(curSupport => {
        ...
        //Read file
        val sequences =
          if (fileName == "BIBLE" || fileName == "Kosarak" || fileName == "test"){
            sc.parallelize(scala.io.Source.fromFile(currentPath).getLines().toSeq.map(x => x.split(" ").map(x=> Array(x))), 300)
          }
          else{
            sc.parallelize(scala.io.Source.fromFile(currentPath).getLines().toSeq.map(x => x.split(" -1 ").filter(x => !x.equals("")).map(x=> x.split(" "))), 300)
          }
        ...
        val prefixSpan = new PrefixSpan()
          ...
          .setOutPutItemSetPermutation(true)
