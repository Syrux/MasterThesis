...
private[fpm] class PPICRunner(val minSup: Long,
  ...
  def findFrequentItems(postfixes: Array[Sequence]): Unit = {
    ...
    for (postfix <- postfixes) {
      // Find frequent items in the sequence (/!\ DON'T skip first item /!\)
  ...
  /**
   * Clean a sequence to remove separator (0).
   *
   * Example :
   * 0 89 0 104 0 104 0 72
   * 0 89 0 104
   ...
   * 89 104
   *
   * @param postfix : A sequence of INT
   * @return The sequence cleaned from un-frequent items
   */
  def preProcessSwitchedArrayToSDB(postfix: Array[Int]): Option[Array[Int]] = {
    ...
    var nbElemAdded = 0
    // Filter items to clean sequence
    val cleanedSequence = postfix.flatMap(x => {
      if (x != 0) {
        // Item supported
        // Update last pos found in current sequence for current item
        nbElemAdded += 1
        lastItemPosInSeq(x) = nbElemAdded
        // If necessary, update first pos in current sequence for current item
        if (firstItemPosInSeq(x) == 0) {
          firstItemPosInSeq(x) = nbElemAdded
        }
        Some(x)
      }
      else None // Separator -> removed
    })
  ...
  /**
   * Find the complete set of frequent sequential patterns in the input sequences.
   * This method is used when switching from multi-item to single item problems
   *
   * @return frequent patterns
   */
  def switch(postfixes: Array[Array[Int]],
             frequentItems: collection.mutable.Map[Int, Int],
             trans: collection.mutable.Map[Int, Int],
             transBack: collection.mutable.Map[Int, Int]): Iterator[(Array[Int], Long)] = {

    // Frequent item
    supportedOriginalItem = frequentItems
    // Init translation maps
    translationMap = trans
    translationMapReverse = transBack
    // Init first and last pos list
    firstItemPosInSid = scala.collection.mutable.ArrayBuilder.make[Array[Int]]
    lastItemPosInSid = scala.collection.mutable.ArrayBuilder.make[Array[Int]]
    ...
    // Clean postfixes, translate items, check if input is correct and
    // Create first / last pos list
    val sdb: Array[Array[Int]] = postfixes.flatMap(x => preProcessSwitchedArrayToSDB(x))
    ...
    // Init constraint variables
    val itemsSupport = Array.tabulate(frequentItems.keySet.size)(i => i).map(i =>
      supportedOriginalItem.getOrElse(translationMapReverse.get(i).get, 0))
    val CPVariables = initCPVariables(itemsSupport)
    ...
    val solutions = scala.collection.mutable.ArrayBuffer.empty[(Array[Int], Long)]
    val c = new PPIC(CPVariables, sdb, sdbPosList,
      firstPosOfItem, lastPosOfItem, itemsSupport, minSup, itemsSupport.length)
