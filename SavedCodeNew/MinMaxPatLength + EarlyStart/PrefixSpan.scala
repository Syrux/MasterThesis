...
class PrefixSpan private (
   ...
   * {minSupport: `0.1`, maxPatternLength: `0`, minPatternLength: `1`, maxLocalProjDBSize: `32000000L`}.
   */
  @Since("1.5.0")
  def this() = this(0.1, 0, 1, 32000000L)
  ...
  def run[Item: ClassTag](data: RDD[Array[Array[Item]]]): PrefixSpanModel[Item] = {
    ...
    val results = genFreqPatterns(dataInternalRepr, frequentItemsAndCounts, minCount, maxPatternLength, minPatternLength, maxLocalProjDBSize)
...
object PrefixSpan extends Logging {
  ...
  private[fpm] def genFreqPatterns(
    ...
    genFreqPatterns(data, Array(), minCount, maxPatternLength, minPatternLength, maxLocalProjDBSize)
  }
  ...
  private[fpm] def genFreqPatterns(
    ...
    for ((item, count) <- freqItems) {
      ...
      // TODO : IF already small enough, directly use local exec
    }
    // Else start with empty prefix.
