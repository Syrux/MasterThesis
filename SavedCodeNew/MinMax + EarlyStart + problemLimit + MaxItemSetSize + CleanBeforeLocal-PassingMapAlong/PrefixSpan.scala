...
class PrefixSpan private (
  ...
  /**
    * Gets the minimal pattern length (i.e. the length of the smallest sequential pattern to consider.
    */
  @Since("1.5.0")
  ...
  /**
    * Gets the soft subProblem limit (i.e. the number of subProblems created after which we force a local exec)
    */
  @Since("1.5.0")
...
object PrefixSpan extends Logging {
  ...
  private[fpm] def genFreqPatterns(
          ...
          largePrefixArray.flatMap { prefix =>
            ...
            val shouldSearchInCurItem =
              if(maxItemPerItemSet > 0 &&
                prefix.items.length - prefix.items.lastIndexOf(0) - 1 >= maxItemPerItemSet) false
              else true
    ...
    if (numSmallPrefixes > 0) {
      ...
      val distributedFreqPattern = postfixes.flatMap { postfix =>
        ...
        // Search
        if (cleaningResult._3) {
          // PPIC
          val localPrefixSpan = new PPICRunner(minCount, newMinPatternLength, newMaxPatternLength, cleaningResult._2)
  ...
  /**
    * @param postfixes : The postfixes to clean
    * @param prefix : The current prefix
    * @param minSupport : The current minSupport
    *
    * @return A tuple containing :
    *           1. The cleaned postfixes (Un-frequent items removed)
    *           2. A map of all frequent items
    *           3. A boolean indicating whether PPIC will be usable or not
    */
  private[fpm] def cleanSequences(postfixes: Iterable[Postfix], prefix: Prefix, minSupport: Long): (Array[Postfix], mutable.Map[Int, Int], Boolean) = {

    //Init
    // Map to collect item frequency
    val frequentItems = collection.mutable.Map[Int, Int]()
    ...
    for (postfix <- postfixes) {
      ...
      itemSupportedByThisSequence.keys.foreach(x =>
        frequentItems.update(x, frequentItems.getOrElse(x, 0) + 1)
      )
      // Clean for next iter
      itemSupportedByThisSequence.clear()
    }
    // Save a copy of frequent items
    val savedFrequentItems = frequentItems.filter(kv => kv._2 >= minSupport)

    // Item in current prefix are supported by default !
    for (item <- prefix.items) {
      frequentItems.update(item, minSupport.toInt + 1)
    }
    ...
    for (postfix <- postfixes) {
      ...
      for (i <- postfix.items.indices){
        ...
        else if (frequentItems.getOrElse(item, 0) >= minSupport) {
          curSeq += item
          lastItemAdded += 1
          isNotEmpty = true
    ...
    // Return
    (cleanedSequences.result(), savedFrequentItems, canUsePPIC)
  }
