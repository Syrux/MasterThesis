...
object PrefixSpan extends Logging {
  ...
  private[fpm] def genFreqPatterns(
    ...
    genFreqPatterns(data.map(seq => (seq, genFirstPosMap(seq), genLastPosMap(seq))), Array(), minCount, maxPatternLength, 0,
  ...
  private[fpm] def genFreqPatterns(
    ...
    genFreqPatterns(data.map(seq => (seq, genFirstPosMap(seq), genLastPosMap(seq))), Array(), minCount, maxPatternLength, minPatternLength,
      0, 0, maxLocalProjDBSize)
  }
