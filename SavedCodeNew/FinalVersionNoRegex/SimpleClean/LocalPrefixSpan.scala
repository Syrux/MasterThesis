...
private[fpm] class LocalPrefixSpan(
                                    ...
                                    val minPatternLength: Int = 0,
                                    val maxItemPerItemSet: Int = 0,
                                    val spaceRemainingInCurrentItem: Int = 0,
                                    val itemConstraints: Array[(Int, String, Int)] = Array.empty)
  ...
  Iterator[(ReversedPrefix, Long)] = {
    ...
    freqItems.toIterator.flatMap { case (item, count) =>
      ...
      // Add pattern to sol if larger than minPatternLength and respect constraint
      if (newPrefix.length >= minPatternLength &&
        PrefixSpan.isPatternRespectingItemConstraints(newPrefix.items, itemConstraints)) {
