 ...
 *
 *                        Please beware that using anything but the default value when not in
 *                        theses circonstances may be harmfull to performance.
  ...
  def run[Item: ClassTag](data: RDD[Array[Array[Item]]]): PrefixSpanModel[Item] = {
    ...
    val renamedItemConstraint = itemConstraintArray.flatMap{
      case (itemName: Item, operator: String, count: Int) =>
        ...
        else Some((itemToInt(itemName) + 1, operator, count))
    }
    ...
    val results = genFreqPatterns(dataInternalRepr, minCount,
      maxPatternLength, maxLocalProjDBSize, minPatternLength, maxItemPerItemSet, subProblemLimit,
      frequentItemsAndCounts, renamedItemConstraint)
...
object PrefixSpan extends Logging {
   ...
   *                        problems on a single machine.
   *
   * @param freqItems A sequence of tuple (frequentItem, occurenceInDb) used to give a
   *                  headstart to the algorithm and avoid the first iteration. If no item are
   *                  specified, the frequent item and their count will be automatically
   *                  discovered.
   *
   *                  If some item are specified, only sequences starting with those particular
   *                  item will be grown.
   *                  (default value : Array.empty (force automatic discovery))
   *
  ...
  private[fpm] def genFreqPatterns(
                                    ...
                                    freqItems: Array[(Int, Long)] = Array.empty,
  ...
  RDD[(Array[Int], Long)] = {
    ...
    if (freqItems.isEmpty || subProblemLimit == 1) {
      ...
      for ((item, count) <- freqItems) {
        ...
        if( ! isPatternCompletlyViolatingItemConstraints(prefix.items, itemConstraints)) {
          largePrefixes += prefix.id -> prefix
        }
        // If respect constraint and minLength, add to solutions
        if (prefix.length >= minPatternLength &&
          isPatternRespectingItemConstraints(prefix.items, itemConstraints)) {
    ...
    if (numSmallPrefixes > 0) {
      ...
      val distributedFreqPattern = postfixes.flatMap { postfix =>
        ...
        if (cleaningResult._2) {
          // Banned Item
          val BannedItemToRemove =
            if (cleaningResult._3) Array.empty[Int]
            else bannedItem.result()
          // PPIC
          val localPrefixSpan = new PPICRunner(minCount, newMinPatternLength, newMaxPatternLength,
            newItemConstraints, BannedItemToRemove)
          ...
          val localPrefixSpan = new LocalPrefixSpan(minCount, newMaxPatternLength,
            newMinPatternLength, maxItemPerItemSet, spaceRemainingInCurrentItem, newItemConstraints)
   ...
   *
   * First, to clean all sequences from items that cannot be part of further solution, thus
   * reducing the size of the database and improving performance. (If done too often it will
   * reduce performance instead, thus why it should be done before the local execution, since a
   * lot of work has already been done)
   *
   * Second, to determine whether the sequence database are suitable to use PPIC. An algorithm whose
   * performance are far better but which is limited to itemSets of one item.
   *
   * Both theses results are returned as a tuple at the end of this method.
   *
   * @param postfixes  Compressed sequences to be cleaned
   * @param prefix  The prefix obtained before compressing the sequence, all item in the
   *                prefix must be supported to ensure that the local execution will
   *                return correct resutls.
   * @param minSupport The minimum number of sequences necessary an item to be supported.
   *
   * @param bannedItems Items banned through item constraints. Must be removed from sequences.
   *
  ...
  private[fpm] def cleanSequences(
    ...
    for (postfix <- postfixes) {
      ...
      val newPartialStarts = mutable.ListBuffer.empty[Int]
      ...
      if (postfix.items.length > 0) {
        ...
        if(postfix.items(0) != 0) numberOfItemPerItemSetCounter += 1
        lastItemAdded += 1
