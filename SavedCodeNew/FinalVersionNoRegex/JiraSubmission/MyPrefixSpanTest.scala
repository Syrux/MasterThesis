...
object MyPrefixSpanTest {
  ...
  def main(args: Array[String]) {
    ...
    //val sequences = sc.parallelize(Seq(Array(Array(1), Array(5, 6), Array(1, 2, 3, 4, 5))), 2).cache()
    //val sequences = sc.parallelize(Seq(Array(Array(1, 2), Array(1, 2, 3))), 2).cache()
    //val curSupport = 1
    ...
    */
    ...
    val currentPath = generalPath + fileName + ".txt"
    val curSupport = 0.1
    val sequences = sc.parallelize(scala.io.Source.fromFile(currentPath).getLines().toSeq.map(x
      => x.split(" ").map(x=> Array(x))), 2)
      */
