...
object PrefixSpan extends Logging {
  ...
  private[fpm] def genFreqPatterns(
    ...
    // Find the longest possible itemSetSize
    val maxItemSizeInInput = postfixes.map(_.maxItemSetSize()).max()
    val maxItemPerItemSetRecalculated =
      if (maxItemPerItemSet > 0) math.min(maxItemSizeInInput, maxItemPerItemSet)
      else maxItemSizeInInput
        ...
        val freqPrefixes = postfixes.flatMap { postfix =>
          largePrefixArray.flatMap { prefix =>
            ...
            val shouldSearchInCurItem =
              if(maxItemPerItemSetRecalculated > 0 &&
                prefix.items.length - prefix.items.lastIndexOf(0) - 1 >= maxItemPerItemSetRecalculated) false
              else true
    ...
    if (numSmallPrefixes > 0) {
      ...
      val distributedFreqPattern = postfixes.flatMap { postfix =>
        ...
        if (maxItemPerItemSetRecalculated == 1){
          ...
          // Spark
          // Setup
          val spaceRemainingInCurrentItem = maxItemPerItemSetRecalculated - (prefix.items.length - prefix.items.lastIndexOf(0) - 1)
          val localPrefixSpan = new LocalPrefixSpan(minCount, newMinPatternLength,
            newMaxPatternLength, maxItemPerItemSetRecalculated, spaceRemainingInCurrentItem)
  ...
  private[fpm] class Postfix(
    ...
    /**
      * Find the maximum itemset size in the remaining sequence.
      *
      * Allow to determine whether a sequence can be run
      * through PPIC or through the general algorithm
      */
    def maxItemSetSize(): Int = {
      // Init variable
      var i = start
      var counter = 0
