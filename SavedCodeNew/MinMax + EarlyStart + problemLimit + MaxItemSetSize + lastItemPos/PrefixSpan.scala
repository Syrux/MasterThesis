  ...
  def run[Item: ClassTag](data: RDD[Array[Array[Item]]]): PrefixSpanModel[Item] = {
    ...
    val dataInternalRepr: RDD[(Array[Int], mutable.Map[Int, Int])] = data.flatMap { itemsets =>
      ...
      if (containsFreqItems) {
        Some((allItems.result(), lastPosMap))
      } else {
        None
      }
  ...
  private[fpm] class Postfix(
    ...
    def project(prefix: Int): Postfix = {
      ...
      if (lastPosMap.getOrElse(math.abs(prefix), -1) <= start) {
        ...
        new Postfix(items, lastPosMap, newStart, newPartialStarts.result())
      }
      else {
    ...
    def compressed: Postfix = {
      if (start > 0) {
        ...
        new Postfix(compressedSeq, newLastPosMap, 0, partialStarts.map(_ - start))
      } else {
        this
      }
    }
