  ...
  def run(postfixes: Array[Postfix]): Iterator[(Array[Int], Long)] = {
    ...
    */
    ...
    val c = new SparkCPOptimised(CPVariables, sdb, lastPrefixItemElems, minSup,
      itemSupportCounter.map{case (a, b) => (a, new ReversibleInt(solver, b))})
    add(c)
...
class SparkCPOptimised(val P: Array[CPIntVar],
  ...
  // A default revesible array to avoid declarration of new ones
  // It should NEVER be modified, only have it's value checked
  private[this] val defaultRevesibleArray = new ReversibleArrayStack[Int](s)
    ...
    while (v < P.length && P(v).isBound && P(v).min != epsilon) {
      ...
      if(v < P.length - 1) {
        ...
        for (value <- P(v + 1)) {
          if (value != separator &&
            value != epsilon &&
            supportMap.getOrElseUpdate(value, new ReversibleInt(s, 0)).value < minsup) {
  ...
  def project(v: Int): Boolean = {
    ...
    val nbSupportForSoughtItem =
      supportMap.getOrElseUpdate(soughtItem, new ReversibleInt(s, 0)).value
    ...
    while (sequenceIndexInPSDB < psdbSize.value && nbAdded < nbSupportForSoughtItem) {
      ...
      val separatorPos = curSeq.getOrElseUpdate(separator, new ReversibleArrayStack[Int](s))
      ...
      if (separatorPos.isEmpty) {
          ...
          if (soughtPos.top < separatorPos.top) {
            ...
            if (posInP == v) {
              ...
              val prefixSearcherArray = Array.fill[ReversibleArrayStack[Int]](v - posInP + 1) {
                ...
                val itemMap = curSeq.getOrElse(item, defaultRevesibleArray)
        ...
        // IF curSeq does not contain soughtPos, kill sequence for future iter
        while (separatorPos.size > 1) separatorPos.pop()
        index.setValue(separatorPos.pop() + 1)
      }
      ...
      if (projectedSuccessfully && !separatorPos.isEmpty) {
        ...
        writeIndex += 1
        nbAdded += 1
        // Advance sequence (-> Give perf improvement)
        val i = index.value
        curSeq.foreach {
          ...
          if (soughtItem == separator) {
            for (value <- P(v + 1)) {
              if (curSeq.contains(value)) {
                // Get item
                val stack = curSeq.get(value).get
            ...
            val prefixSearcherArray = Array.fill[ReversibleArrayStack[Int]](v - posInP + 1) {
              // Find itemMap
              val item = getElemOfP(posInP)
              val itemMap = curSeq.getOrElse(getElemOfP(posInP), defaultRevesibleArray)
              // Prepare next item
              posInP += 1
              // Return
              itemMap
