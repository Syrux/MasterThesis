tiff("FinalVersionNoRegex.png", width=18, height=9, units='in', res=120)
...
four <- c(22525, 20743, 21401, 22428, 23459, 28200, 33593, 42280, 58723, 143557)/1000
five <- c(23369, 20175, 21040, 23738, 24623, 29845, 35764, 44398, 60009, 145250)/1000
...
grid()
legend("bottomright", legend=c("BaseSpark", "BasePPIC", "MmEsLpMiss_simpleClean", "MmEsLpMiss_cleanThenTransfer", "FinalVersionNoRegex_DC", "FinalVersionNoRegex_SC"), lty=c(1,1),lwd=c(2,2), col=c("blue", "red", "magenta", "green", "black", "darkgoldenrod"), cex = 1)
...
one <- c(359711, 490719, 613541, 726131)/1000
two <- c(307324, 445807, 569527, 715469)/1000
three <- c(313665, 446323, 556453, 694012)/1000
four <- c(325769, 462997, 606735, 740408)/1000
five <- c(307021, 444755, 570330, 719730)/1000

#PLOT
plot(x, zero, log = "xy", type="o", pch=0, xlab="Minsup(%)",ylab="Time (s logscale)", col="blue", xlim = rev(range(x)), ylim=c(307, 740))
lines(x, one, col="red", type="o", pch=1)
lines(x, two, col="magenta", type="o", pch=4)
lines(x, three, col="green", type="o", pch=5)
lines(x, four, col="black", type="o", pch=6)
...
one <- c(10592, 10615, 11709, 12472, 13241, 15153)/1000
two <- c(16688, 17908, 18961, 20953, 22498, 25327)/1000
three <- c(15886, 17709, 18883, 20282, 21138, 24560)/1000
four <- c(15588, 17048, 17596, 19098, 20958, 23497)/1000
five <- c(16638, 18229, 19110, 20317, 22155, 25099)/1000

#PLOT
plot(x, zero, log = "xy", type="o", pch=0, xlab="Minsup(%)",ylab="Time (s logscale)", col="blue", xlim = rev(range(x)), ylim=c(10, 25))
lines(x, one, col="red", type="o", pch=1)
lines(x, two, col="magenta", type="o", pch=4)
lines(x, three, col="green", type="o", pch=5)
lines(x, four, col="black", type="o", pch=6)
...
four <- c(2510, 2577, 2677, 2725, 3081, 3125, 3601, 3887, 5504, 7288)/1000
five <- c(2822, 2782, 2842, 3052, 3397, 3503, 4052, 4414, 5416, 8141)/1000
...
four <- c(8307, 8559, 9442, 10633, 14689, 22462)/1000
five <- c(8956, 9336, 10249, 11539, 16225, 25284)/1000
...
four <- c(20683, 22658, 26916, 30191, 34036, 41168, 52060, 74124)/1000
five <- c(23201, 24925, 30657, 33265, 38083, 44532, 56105, 78123)/1000
...
four <- c(16903, 18691, 21227, 23410, 25048, 26062, 29873, 31750, 32163)/1000
five <- c(16148, 17981, 21171, 22193, 24595, 25519, 29340, 31913, 32135)/1000
...
grid()
legend("bottomright", legend=c("BaseSpark", "MmEsLpMiss_simpleClean", "MmEsLpMiss_cleanThenTransfer", "FinalVersionNoRegex_DC", "FinalVersionNoRegex_SC"), lty=c(1,1),lwd=c(2,2), col=c("blue", "magenta", "green", "black", "darkgoldenrod"), cex = 1)
...
four <- c(20289, 22537, 25372, 28287, 31163, 32456, 36837, 42475, 45137)/1000
five <- c(19346, 21970, 24495, 27409, 31411, 31715, 35497, 41780, 44965)/1000
