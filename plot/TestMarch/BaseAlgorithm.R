tiff("BaseAlgorithm.png", width=18, height=9, units='in', res=120)
...
grid()
legend("bottomright", legend=c("BaseSpark", "BasePPIC"), lty=c(1,1),lwd=c(2,2), col=c("blue", "red"), cex = 1)
...
#PLOT
plot(x, zero, log = "xy", type="o", pch=0, xlab="Minsup(%)",ylab="Time (s logscale)", col="blue", xlim = rev(range(x)), ylim=c(273, 536))
lines(x, one, col="red", type="o", pch=1)
...
grid()
legend("bottomright", legend=c("BaseSpark"), lty=c(1,1),lwd=c(2,2), col=c("blue"), cex = 1)
...
#PLOT
plot(x, zero, log = "xy", type="o", pch=0, xlab="Minsup(%)",ylab="Time (s logscale)", col="blue", xlim = rev(range(x)), ylim=c(13, 34))
