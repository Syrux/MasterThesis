...
\begin{algorithm}[!h]
    ...
    \SetKwFunction{Scalable}{scalableExecution}
   	...
   	\Fn{\Scalable{$database, minPatLen, maxPatLen, maxLocalSize, \theta,$ \\ $limitItemPerItemset, softSubProblemLimit, freqItemAndCount, symbolConstraints$}} {
		\tcc{Encapsulate each sequence in a postfix, allowing to keep the current position and partial starts without copying the sequences. For SoS database, use the shortened database representation for the PostFixes.}
		postfixes = $\emptyset$ \\
		...
		\While {largePrefixes is not empty}{
			\tcp{Project and extend prefix through a map-reduce}
			...
			largePrefixes = projectAndExtend(largePrefixes)\\
		}
