...
class PrefixSpan private (
   ...
   * (i.e. the soft limit on the number of subProblems created, before which we force a local exec)
   */
  @Since("1.5.0")
   ...
   */
  @Since("1.5.0")
  def run[Item, Itemset <: jl.Iterable[Item],
  Sequence <: jl.Iterable[Itemset]](data : JavaRDD[Sequence]): PrefixSpanModel[Item] = {
...
object PrefixSpan extends Logging {
    ...
    if (numSmallPrefixes > 0) {
      ...
      val distributedFreqPattern = postfixes.flatMap { postfix =>
        ...
        if (cleaningResult._2) {
          ...
          // TODO: We collect projected postfixes into memory. We should also compare the
          // TODO: performance of keeping them on shuffle files.
  ...
  private[fpm] class Postfix(
     ...
     *      postfix can be assembled to `<1>` is `_2` and `_3`, resulting new prefixes `<(12)>`
     *      and `<(13)>`.
     *      We flip the sign in the output to indicate that this is a partial prefix item.
     ...
     *      and `<13>`.
     * @return an iterator of (prefix item, corresponding postfix size). If the item is negative,
     *         it indicates a partial prefix item, which should be assembled to the last itemset
     *         of the current prefix. Otherwise, the item should be appended to the current prefix.
     */
